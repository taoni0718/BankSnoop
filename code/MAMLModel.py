...
class MAML:
    ...
    def get_maml_model(self):
        ...
        ## CNN model
        model = models.Sequential([
            layers.Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=self.input_shape),
            layers.BatchNormalization(),
    ...
    def train_on_batch_mag(self, train_data, inner_optimizer, inner_step, outer_optimizer=None):
        """
        MAML一个batch的训练过程
        ...
        meta_support_mag, meta_support_label, meta_query_mag, meta_query_label = next(train_data)
        for support_mag, support_label in zip(meta_support_mag, meta_support_label):

            # 每个task都需要载入最原始的weights进行更新
            self.meta_model.set_weights(meta_weights)
            for _ in range(inner_step):
                with tf.GradientTape() as tape:
                    logits = self.meta_model(support_mag, training=True)
        ...
        with tf.GradientTape() as tape:
            for i, (query_mag, query_label) in enumerate(zip(meta_query_mag, meta_query_label)):

                # 载入每个task weights进行前向传播
                self.meta_model.set_weights(task_weights[i])

                logits = self.meta_model(query_mag, training=True)
